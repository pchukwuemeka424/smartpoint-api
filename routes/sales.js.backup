...
router.get('/', auth, async (req, res) => {
    try {
        ...
        // Build query based on user role - use fallback to ensure we find transactions
        let query = {
            $or: [
                { managerId: req.user.id },
                { cashierId: req.user.id },
...
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        const sale = await Sale.findOne(query)
            .populate('items.item', 'name category brand sku')
            .populate('cashierId', 'firstName lastName role')
            .populate('managerId', 'firstName lastName role');
...
router.get('/daily/:date', auth, async (req, res) => {
    try {
        ...
        // Use role-based filtering for daily sales
        let sales;
        if (req.user.role === 'manager') {
            ...
            sales = await Sale.getDailySales(req.user.id, targetDate);
        }
